...
not200DF = logs_df.where(logs_df['status']!='200').groupBy('path').count()
not200DF.show(10)
# Sorted DataFrame containing all paths and the number of times they were accessed with non-200 return code
logs_sum_df = not200DF.orderBy(not200DF['count'].desc())
...
# TODO: Replace <FILL IN> with appropriate code
import pyspark.sql.functions as d


#unique_host_count = (logs_df.agg(d.countDistinct('host')))['count(host)'][1]
...
days_with_hosts = daily_hosts_df.map(lambda s: s[0]).collect()
hosts =daily_hosts_df.map(lambda s: s[1]).collect()
...
dayAndHostTuple = logs_df.map(lambda log: (log.time.day, log.host))
groupedByDay = dayAndHostTuple.groupByKey()
sortedByDay = groupedByDay.sortByKey()
avg_daily_req_per_host_df = sortedByDay.map(lambda x: (x[0], len(x[1])/len(set(x[1])))).cache()
avg_daily_req_per_host_list  = avg_daily_req_per_host_df.take(30)
...
Test.assertEquals(avg_daily_req_per_host_list, [(1, 13), (3, 12), (4, 14), (5, 12), (6, 12), (7, 13), (8, 13), (9, 14), (10, 13), (11, 14), (12, 13), (13, 13), (14, 13), (15, 13), (16, 13), (17, 13), (18, 13), (19, 12), (20, 12), (21, 13), (22, 12)], 'incorrect avgDailyReqPerHostList')
Test.assertTrue(avg_daily_req_per_host_df.is_cached, 'incorrect avgDailyReqPerHost.is_cache')
...
# TODO: Replace <FILL IN> with appropriate code
display(avg_daily_req_per_host_list)
...
top_20_not_found_df = top_20_not_found_df.orderBy(top_20_not_found_df['count'].desc())
...
hosts_404_count_df = hosts_404_count_df.orderBy(hosts_404_count_df['count'].desc())
...
errors_by_date_sorted_df = not_found_df.select(dayofmonth('time').alias('day'),not_found_df['status'],not_found_df['path'])
errors_by_date_sorted_df  = (errors_by_date_sorted_df.groupby(errors_by_date_sorted_df['day'])).count().cache()
...
top_err_date_df = errors_by_date_sorted_df.orderBy(errors_by_date_sorted_df['count'].desc())
...
hourCountPairTuple = not_found_df.map(lambda log: (log.time.hour, 1))
hourRecordsSum = hourCountPairTuple.reduceByKey(lambda a, b: a + b)

hourRecordsSorted = (hourRecordsSum.sortByKey()).cache()

hour_records_sorted = hourRecordsSorted.collect()
...
Test.assertEquals(hour_records_sorted, [(0, 175), (1, 171), (2, 422), (3, 272), (4, 102), (5, 95), (6, 93), (7, 122), (8, 199), (9, 185), (10, 329), (11, 263), (12, 438), (13, 397), (14, 318), (15, 347), (16, 373), (17, 330), (18, 268), (19, 269), (20, 270), (21, 241), (22, 234), (23, 272)], 'incorrect hour_records_sorted_df')
...
hours_with_not_found = hourRecordsSorted.map(lambda s:s[0]).collect()
not_found_counts_per_hour = hourRecordsSorted.map(lambda s:s[1]).collect()
